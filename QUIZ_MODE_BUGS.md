...
**Root cause for #1, #2, #4:** answer validation (`validateAnswerPerConstraint`) only checks the per-question *constraints*. It never sees the pre-game *filter* at all — the function signature doesn't even take it (`quizQuestionGenerator.ts:341`). The baseline-query builder (`buildBaselineQuery`, `quizQuestionGenerator.ts:56`) now applies legendary, mythical **and** dual-type (Bug 1 fix), but those baseline filters are still only honored at *generation*, never re-checked when an answer is *graded*. So the remaining defect (Bug 2) is purely on the validation side: every baseline filter shapes the question pool correctly but a hand-typed answer that fits the visible constraints can still slip past a disallowed legendary/mythical/dual-type.
...
**Fix applied (generation path, low risk).** The filter is now applied in the baseline, alongside the legendary/mythical lines (`buildBaselineQuery`, `quizQuestionGenerator.ts:65-67`):
...
2. **Bug 2** — pass `filter` into validation so legendary/mythical/**dual-type** are re-checked at grading; needs your call on visible-vs-silent rows. (After Bug 1, dual-type joins legendary/mythical in this single unified validation gap.)
